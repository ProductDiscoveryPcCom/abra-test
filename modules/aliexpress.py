        ...
        for p in raw_products:
            try:
                ...
                product = AliExpressProduct(
                    product_id=str(getattr(p, 'product_id', '') or getattr(p, 'productId', '')),
                    title=str(getattr(p, 'product_title', '') or getattr(p, 'productTitle', '')),
                    image_url=str(getattr(p, 'product_main_image_url', '') or 
                                 getattr(p, 'productMainImageUrl', '')),
                    price=self._parse_price(getattr(p, 'target_sale_price', 0) or 
                                           getattr(p, 'targetSalePrice', 0)),
                    original_price=self._parse_price(getattr(p, 'target_original_price', 0) or
                    ...
                    orders=self._parse_orders(getattr(p, 'lastest_volume', 0) or
                                             getattr(p, 'lastestVolume', 0)),
                    rating=float(getattr(p, 'evaluate_rate', 0) or 
                                getattr(p, 'evaluateRate', 0) or 0),
                    shop_name=str(getattr(p, 'shop_name', '') or 
                                 getattr(p, 'shopName', '') or 'Unknown'),
                    category=str(getattr(p, 'first_level_category_name', '') or
                                getattr(p, 'firstLevelCategoryName', '') or ''),
                    affiliate_link=str(getattr(p, 'promotion_link', '') or
