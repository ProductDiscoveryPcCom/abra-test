...
def main():
    ...
    with st.sidebar:
        ...
        timeframe = st.selectbox(
            ...
            index=0,  # Default: 5 años
            help="El gráfico tiene zoom interactivo para ver períodos específicos"
        )
        ...
        with st.expander("🔧 Opciones de búsqueda"):
            category = st.selectbox(
                ...
                index=1,  # Por defecto: Informática
                help="Filtrar por categoría para resultados más precisos"
            )
            ...
            exact_match = st.checkbox(
                "Búsqueda exacta",
                value=True,
                help="Usar comillas para buscar la marca exacta (recomendado para marcas con nombres comunes)"
            )
            st.session_state.exact_match = exact_match

            show_volume_estimate = st.checkbox(
                "Mostrar volumen estimado",
                value=False,  # Desactivado por defecto - mejor mostrar índice real
                help="⚠️ Las estimaciones son aproximadas. Para volúmenes reales usa Google Ads Keyword Planner"
            )
            st.session_state.show_volume_estimate = show_volume_estimate

            if show_volume_estimate:
                st.warning("⚠️ Los volúmenes son ESTIMACIONES basadas en heurísticas, NO datos reales de Google.")

            st.caption("💡 El índice de Google Trends (0-100) es el único dato real. Los volúmenes son aproximaciones.")
